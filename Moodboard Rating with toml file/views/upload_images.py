...
bucket_name = 'fashion-tech-moodboards'
...
def insert_image_metadata(sno, image_filename,gcs_url=None, status=None, image_feedback=None):
    try:
        conn = psycopg2.connect(**db_connection)
        cursor = conn.cursor()
        query = """
        INSERT INTO images (sno, image,gcs_url, status, image_feedback)
        VALUES (%s, %s, %s, %s, %s);
        """
...
def update_image_metadata(sno, image_filename, status='PENDING', image_feedback=None, gcs_url=None):
    try:
        conn = psycopg2.connect(**db_connection)
        cursor = conn.cursor()
        query = """
        UPDATE images
        SET image = %s,  status = %s, image_feedback = %s, gcs_url = %s 
        WHERE sno = %s;
        """
        cursor.execute(query, (image_filename, status, image_feedback, gcs_url, sno))
        conn.commit()
        cursor.close()
        conn.close()
...
def get_next_serial_number():
    query = text("SELECT COALESCE(MAX(sno), 0) + 1 AS next_sno FROM images")
        ...
        if new_submit_button:
            if new_uploaded_file:
                try:
                    ...
                    gcs_image_path = f"images/{unique_filename}"
                ...
                if update_sno.isdigit():
                    ...
                    try:
                        ...
                        def check_sno_exists(sno):
                            query = text("SELECT 1 FROM images WHERE sno = :sno")
                            with engine.connect() as conn:
...
# Display data from the 'images' table
...
if df is not None:
    ...
    st.write("Data from the 'images' table(sorted by 'sno'):")
