...
def update_image_metadata(sno, image_filename, status='PENDING', image_feedback=None, gcs_url=None):
    try:
        ...
        query = """
        UPDATE images
        SET image = %s, gcs_url = %s, status = %s, image_feedback = %s
        WHERE sno = %s;
        """
...
def insert_prompt(sno, image_prompt, prompt_feedback=0, prompt_status="Pending"):
    try:
        ...
        cursor = conn.cursor()
        query = """
        INSERT INTO prompts (sno, prompt_feedback, image_prompts, status)
        VALUES (%s, %s, %s, %s);
        """
...
def get_prompts(sno):
    try:
        conn = psycopg2.connect(**db_connection)
        cursor = conn.cursor()
        query = "SELECT image_prompts FROM prompts WHERE sno = %s;"
...
def check_serial_exists(sno):
    try:
        conn = psycopg2.connect(**db_connection)
        cursor = conn.cursor()
        query = "SELECT COUNT(*) FROM images WHERE sno = %s;"
        cursor.execute(query, (sno,))
